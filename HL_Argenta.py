...
try:
    data_as_frameList = tabula.read_pdf("https://www.argenta.be/content/dam/argenta/documents/emprunter/credit-logement/Feuille de tarifs Crédits hypothécaires.pdf",
    ...
    if data_as_frameList:
        try:
            ...
            for i in range(len(data.values.tolist())):
                fixed_rate_matrix.append(
                    ["ARGENTA", "HOME LOAN", "FIXED RATE", data.values.tolist()[i][1], data.values.tolist()[i][3]])
